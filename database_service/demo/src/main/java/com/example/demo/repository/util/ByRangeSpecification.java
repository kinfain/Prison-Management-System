...
public class ByRangeSpecification<T> implements Specification<T> {
	  private final List<EntityRange<T>> ranges;
	 
	  public ByRangeSpecification(List<EntityRange<T>> ranges) {
	    this.ranges = ranges;
	  }
	  ...
	  public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
	    ...
	    for (EntityRange<T> range : ranges) {
	      if (range.isSet()) {
	  ...
	  private Predicate buildRangePredicate(EntityRange<T> range, Root<T> root, CriteriaBuilder builder) {
	    if (range.isBetween()) {
